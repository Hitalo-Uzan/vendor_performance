...
bq_query = """
DELETE FROM `RBBR_PROD.PERFORMANCE_SELLER` 
WHERE DATA_REF >= DATE_SUB(CURRENT_DATE('America/Sao_Paulo'),INTERVAL 7 DAY);

INSERT INTO RBBR_PROD.PERFORMANCE_SELLER 

WITH DB_FATURAMENTO AS (
SELECT 
    CASE WHEN CLIENTE_SELLOUT IS NOT NULL THEN CLIENTE_SELLOUT
    WHEN CAT_CLIENTE = '%VENDA E-COMMERCE%' THEN 'E-COMMERCE'
    WHEN CAT_CLIENTE = 'FEIRA' THEN 'FEIRA'
    WHEN GRP_CLIENTE != '' THEN GRP_CLIENTE
    ELSE FAT.NM_EXECUTIVO_CONTAS END AS CLIENTE
  ...
  ON DMC.NM_CLIENTE = IF(FAT.GRP_CLIENTE != '', FAT.GRP_CLIENTE, FAT.NM_RAZAO_SOCIAL)
